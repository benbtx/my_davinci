...
export class Main extends React.Component<IMainProps, {}> {
  ...
  public render() {
    //获取url 包含 http://localhost:5002/?#/login?yh=admin&psd=admin123456
    //则退出 删除 removeToken
    // const { logged, navigator } = this.props
    var { logged, navigator } = this.props


    // if(location.href.indexOf('psd')>-1){
    //   logged=false
    //   // removeToken()
    // }
    if((location.href.indexOf('id')>-1&&location.href.indexOf('name')>-1)||location.href.indexOf('token')>-1){
        logged=false
      // removeToken()
    }
