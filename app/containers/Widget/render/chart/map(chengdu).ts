...
export default function (chartProps: IChartProps) {
  ...
  data.forEach((record) => {
    ...
    cols.forEach((col) => {
      ...
      if (visualType === 'geoProvince') {
        ...
        if (area) {
          if (layerType === 'map') {
            ...
            dataTree[area.name].value += value
      ...
      } else if (visualType === 'geoDistrict'||visualType === 'string') {
        areaVal = record[col.name]
        const area = getCityArea(areaVal)
        if (area) {
          if (layerType === 'map') {
  ...
  metricOptions = {
    ...
    //   itemStyle: {
    //     normal: {
    //         areaColor: 'red'//json 地图块颜色
    //     },
    //     emphasis: {
    //         areaColor: '#fff'//moveover颜色
    //     }
    //   }
    // },
    // backgroundColor: '#000',//全局地图背景色
    //sl
  }
...
function getCityArea (name) {
  ...
  console.log(geoData)
  console.log(geoData.find((d) => d.name === name))
  console.log(geoData.find((d) => d.name.includes(name)))
